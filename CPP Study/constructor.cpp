...
void Matrix::GetData(int row, int col)
{
	for (int i = 0; i < row; i++)
		for (int j = 0; j < col; j++)
			*(array + row * i + j) = rand() % 10;
}

void Matrix::ShowData(int row, int col)
{
	for (int i = 0; i < row; i++)
	{
		for (int j = 0; j < col; j++)
			printf("%4d", *(array + row * i + j));
		cout << endl;
	}
	cout << endl;
}

void Matrix::Multiply(Matrix& b, Matrix& c, int acol, int brow) // acol should be 
{
	int sum = 0;
	if (acol == brow)
	{
...
int main()
{
	...
	A.GetData(AROWS, ACOLS);
	B.GetData(BROWS, BCOLS);
	cout << "Matrix A" << endl << endl;
	A.ShowData(AROWS, ACOLS);
	cout << "Matrix B" << endl << endl;
	B.ShowData(BROWS, BCOLS);
